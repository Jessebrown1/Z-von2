...
// the Paystack popup.
router.post('/initialize', optionalAuth, express.json(), (req, res) => {
  try {
    ...
    const totals = computeAmount(items);
...
router.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  if (event.event === 'charge.success' && event.data?.reference) {
    markOrderPaid(event.data.reference, event.data);
  }
